...
export async function GET() {
    ...
    const rows = getDatabaseConnection().prepare('SELECT id, name, email, role, permissions_json AS permissions, is_active AS isActive, created_at AS createdAt FROM users ORDER BY email').all().map(row => ({ ...row, permissions: JSON.parse(row.permissions || '[]') }));
