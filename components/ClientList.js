...
export default function ClientList({ clients = [], projects = [], authUser, onRefresh }) {
  ...
  const mergedClients = useMemo(() => {
    ...
    for (const c of clients) {
      const stats = projectStats[c.id] || projectStats[c.email] || { projectCount: 0, totalSpend: 0, lastActive: null, currency: getGlobalCurrency() };
      merged.set(c.id, {
        ...c,
    ...
    for (const [key, c] of Object.entries(credClients)) {
      ...
      if (match) continue;
      const stats = projectStats[key] || { projectCount: 0, totalSpend: 0, lastActive: null, currency: getGlobalCurrency() };
      merged.set(c.id, {
        ...c,
        projectCount: stats.projectCount,
        totalSpend: stats.totalSpend,
  ...
  const hasClients = mergedClients.length > 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {hasClients ? `Active directory (${mergedClients.length} total)` : 'Active directory (0 total)'}
          </p>
        </div>
        {canWrite && (
          <button
            ...
            <table className="w-full caption-bottom text-sm text-left min-w-[800px]">
              <thead className="[&_tr]:border-b [&_tr]:border-border bg-muted/20">
                <tr className="border-b border-border transition-colors">
                  ...
                  <th className="h-10 px-4 align-middle font-medium text-muted-foreground whitespace-nowrap">Email / Phone</th>
                  ...
                  <tr key={client.id} className="border-b border-border transition-colors hover:bg-muted/30">
                    ...
                    <td className="p-4 align-middle text-muted-foreground">
                      {client.email && client.email !== 'N/A' ? (
                        <a href={`mailto:${client.email}`} className="hover:text-primary truncate block max-w-[200px]">
                          {client.email}
                        </a>
                      ) : '—'}
                    </td>
                    ...
                    <td className="p-4 align-middle font-mono">
                      {formatCurrency(client.totalSpend || 0, client.currency || getGlobalCurrency())}
                    </td>
                      ...
                      <td className="p-4 align-middle text-right whitespace-nowrap">
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="app-button-icon disabled:opacity-40"
                          >
                            <Edit2 size={14} />
                          </button>
                          <button
