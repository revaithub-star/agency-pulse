...
export function getUserByEmail(email) {
    return getDatabaseConnection().prepare(`
        SELECT id, email, password_hash, role, permissions_json, is_active FROM users WHERE email = ?
    `).get(String(email || '').trim().toLowerCase());
}

export function getUserById(id) {
    return getDatabaseConnection().prepare(`
        SELECT id, email, role, permissions_json, is_active FROM users WHERE id = ?
    `).get(id);
}
...
export function createUser({ email, password, role = 'staff', permissions = defaultPermissions }) {
    ...
    getDatabaseConnection().prepare(`
        INSERT INTO users (id, email, password_hash, role, permissions_json, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(user.id, user.email, user.passwordHash, user.role, JSON.stringify(user.permissions), now, now);
    return user;
}
